...
2. Our firmware centers on the ATmega328PB, which manages the system’s two main modes: time display and animation. In time mode, the microcontroller reads the current time from the RTC over I²C, and should convert it into the corresponding activation pattern for the 3×5 electromagnet array (we have yet to do this), and updates the display; in animation mode, it instead steps through predefined electromagnet sequences to generate motion (we have yet to do this). The firmware also reads the hall-effect sensor, updates the LCD with status information, and uses button-triggered interrupts to switch modes quickly and reliably. The most important drivers we wrote were the I²C driver for communication with the RTC and solenoid-control hardware, the RTC driver for register-level time reads and writes, the button interrupt logic for mode changes.
   ...
   6 &7.
3. ![1776449890355](image/README/1776449890355.png)
...
| SRS-06 | The system shall detect button presses through hardware interrupts to switch between time display mode and animation mode.                                                                                                        | The final prototype toggles time and draw modes by polling the mode switch on PD2 in the main loop, not by button-driven hardware interrupts; behavior was verified by changing the switch and. observing the LCD mode string and grid behavior update accordingly.                   |
...
| HRS-07 | The system shall include two push buttons for selecting operating modes by interrupting the MCU and an LCD display connected through I2C to show the system status and current mode.                                     | The system meets HRS-07’s functional goal by using a user input to switch modes and an I2C LCD to display system status. In the implemented firmware, mode selection is done by**polling** a digital input (`PD2`) in the main loop, not by button-triggered hardware interrupts. This still provides reliable mode control (`Mode: Time` / `Mode:Draw`) and live ADC status on the LCD. Interrupts are used elsewhere (Timer1 millisecond tick), while button handling uses polling in this build. |

### 4. Conclusion

Overall, this project taught us how challenging it is to integrate software, electronics, power delivery, sensing, and mechanical fabrication into one working embedded system. We learned how to control a 3x5 electromagnet array using I2C solenoid driver boards, read sensor feedback from a Hall effect sensor through the ADC, use an RTC module for timekeeping, update an LCD display, and handle button-based mode switching with interrupts. One part that went well was the modular development process: we were able to separately test the electromagnet array, the peripheral system, and the user interface before combining them into the final prototype. We are especially proud that we built a full physical system that could activate electromagnets in controlled patterns, display live system status, and produce measurable Hall sensor readings. This showed that our core idea of programmable magnetic field control was feasible.

At the same time, the project required us to change our approach several times. We redesigned the mechanical enclosure after the first press-fit design produced an uneven electromagnet surface, switching to a screw-mounted design with threaded inserts. Some obstacles, such as the complexity of wiring 15 electromagnets, managing high-current switching, and keeping the ferrofluid contained cleanly, were more difficult than expected. If we could do the project again, we would start the mechanical and fluid-containment testing earlier, make the wiring harness more organized, and design a cleaner PCB or custom connector system instead of relying heavily on breadboard wiring.

The next step for this project would be to make the system more robust and closer to a real experimental platform. This could include designing a custom PCB or adding stronger closed-loop feedback using multiple Hall sensors. A future version could also test different magnetic materials, stronger electromagnets, and smoother motion-control. The final prototype successfully demonstrated the main concept of using an embedded system to generate programmable magnetic patterns, and it gave us valuable experience in debugging a complex hardware-software project from design to final demo.
